...
public class Voxel : MonoBehaviour {
     ...
     * color
     ...
     red (1, 0, 0, 1).
     white (1, 1, 1, 1);
     yellow(1, 0.92f, 0.016f, 1);

      Color 
      color1 = new Color(0, 1, 1, 1);//cyan
      Color color2 = new Color(1, 0, 0, 1);//red
     */
    ...
    /// </summary>
    /// <param name="_VoxelDisplayLayerDensity"></param>
    public void VoxelDisplayLayerDensity(float _layerdensity, float _minlayerdensity,float _maxlayerdensity)
    {
        if (state == 1)
        {
            ...
            float mappedvalue = Remap(_layerdensity, _minlayerdensity, _maxlayerdensity, 0.0f, 1.0f);
            //two colors to interpolate between
            Color color1 = new Color(0, 0, 1, 1);
            Color color2 = new Color(1, 0.92f, 0.016f, 1);
            //interpolate color from mapped value
            Color mappedcolor = Color.Lerp(color1, color2, mappedvalue);
            props.SetColor("_Color", mappedcolor);
            // Updated the mesh renderer color
